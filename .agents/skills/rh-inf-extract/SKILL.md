...
> and `.agents/skills/rh-inf-extract/reference.md` only. Inspecting source code wastes context and introduces
...
> documented in `SKILL.md` and `.agents/skills/rh-inf-extract/reference.md`. Calling `--help` wastes context
...
> prompt. Only read `.agents/skills/rh-inf-extract/reference.md` and `.agents/skills/rh-inf-extract/examples/*.md` on demand — those are
...
`.agents/skills/rh-inf-extract/reference.md` (Terminology Resolution).
...
For detailed mechanics, see `.agents/skills/rh-inf-extract/reference.md` (Terminology Resolution).
...
3. For each approved artifact other than `concepts`, construct the artifact YAML by reasoning over the
   normalized source files and the schema for the artifact type (see `.agents/skills/rh-inf-extract/reference.md`).
   ...
   apply it explicitly:
   - `decision-table`: read the `decision-table` schema block in `.agents/skills/rh-inf-extract/reference.md`
   ...
   - `care-pathway`: read the `care-pathway` schema block and guidance bullets
     in `.agents/skills/rh-inf-extract/reference.md` before editing the body. Use the flat `steps[]` +
   ...
   - `evidence-summary`: read the matching schema block in `.agents/skills/rh-inf-extract/reference.md` and
   ...
   - `terminology`: read the matching schema block in `.agents/skills/rh-inf-extract/reference.md` and keep
     the output constrained to explicit value-set / concept-map structure backed
     by the approved concept review results.
   - `measure`: read the matching schema block in `.agents/skills/rh-inf-extract/reference.md` and express
     population definitions, scoring method, and improvement notation explicitly.
   - `assessment`: read the matching schema block in `.agents/skills/rh-inf-extract/reference.md` and express
     instrument metadata, items, options, scoring, and MCP-derived codings when available.
   - `policy`: read the matching schema block in `.agents/skills/rh-inf-extract/reference.md` and separate
     applicability, criteria, and disposition actions clearly.
   - Other artifact types: read the matching schema block in `.agents/skills/rh-inf-extract/reference.md`
     before editing the body.

   For `decision-table` and `care-pathway`, also consult `.agents/skills/rh-inf-extract/examples/output.md`
